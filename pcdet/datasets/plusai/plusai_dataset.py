...
class PlusAIDataset(DatasetTemplate):
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                annotations['dimensions'] = np.array([label['box3d_lidar'][3:6] for label in obj_labels])  # lwh(lidar) format
                annotations['location'] = np.array([label['box3d_lidar'][0:3] for label in obj_labels])
                annotations['rotation_y'] = np.array([label['box3d_lidar'][6] for label in obj_labels])
...
def create_plusai_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = PlusAIDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
def create_leading_test_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = PlusAIDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    leading_test_split = 'leading_test'

    val_filename = save_path / ('plusai_infos_%s.pkl' % leading_test_split)

    print('---------------Start to generate data infos---------------')

    dataset.set_split(leading_test_split)
    plusai_infos_leading_test = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=False)
    with open(val_filename, 'wb') as f:
        pickle.dump(plusai_infos_leading_test, f)
    print('PlusAI info val file is saved to %s' % val_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_plusai_infos':
        ...
        create_plusai_infos(
            ...
            class_names=['Car', 'Truck', 'Tram'],
            data_path=Path('/home/tong.wang/datasets/PlusAI/'),
            save_path=Path('/home/tong.wang/datasets/PlusAI/')
        )
    elif sys.argv.__len__() > 1 and sys.argv[1] == 'create_leading_test_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
        dataset_cfg = EasyDict(yaml.load(open(sys.argv[2])))
        create_leading_test_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Truck', 'Tram'],
            data_path=Path('/home/tong.wang/datasets/PlusAI/'),
            save_path=Path('/home/tong.wang/datasets/PlusAI/')
