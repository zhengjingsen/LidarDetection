...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        """
        assert xyz.shape[0] == xyz_batch_cnt.sum(), 'xyz: %s, xyz_batch_cnt: %s' % (str(xyz.shape), str(xyz_batch_cnt))
...
class FeatureFurthestPointSampling(Function):
    @staticmethod
    def forward(ctx, point_features: torch.Tensor, npoint: int):
        """
        Args:
            ctx:
            xyz: (B, N, c) where N > npoint
            npoint: int, number of features in the sampled set

        Returns:
            output: (B, npoint) tensor containing the set
        """
        assert point_features.is_contiguous()

        B, N, C = point_features.size()
        output = torch.cuda.IntTensor(B, npoint)
        temp = torch.cuda.FloatTensor(B, N).fill_(1e10)

        pointnet2.feature_furthest_point_sampling_wrapper(B, N, npoint, C, point_features, temp, output)
        return output
...
feature_furthest_point_sample = FeatureFurthestPointSampling.apply
