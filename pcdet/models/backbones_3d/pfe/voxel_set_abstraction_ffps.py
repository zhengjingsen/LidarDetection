...
class VoxelSetAbstractionFFps(nn.Module):
    ...
    def get_sampled_points(self, points, points_feature, batch_cnt, num_keypoints):
        ...
        for cur_cnt in batch_cnt.cpu().numpy():
            ...
            cur_pt_idxs = pointnet2_stack_utils.feature_furthest_point_sample(
                features_for_fps, num_keypoints
            ).long()
    ...
    def forward(self, batch_dict):
        ...
        points, point_features, batch_cnt = self.get_points_for_sample(batch_dict)
        point_features = point_features.new_zeros([point_features.shape[0], 0])
        point_features_list = []
